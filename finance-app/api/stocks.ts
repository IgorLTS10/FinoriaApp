// api/stocks.ts
import type { VercelRequest, VercelResponse } from "@vercel/node";
import { db } from "../src/db/client.js";
import { stockPositions } from "../src/db/schema.js";
import { eq } from "drizzle-orm";

export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    // ============================================================
    // GET — liste des actions d’un utilisateur
    // /api/stocks?userId=123
    // ============================================================
    if (req.method === "GET") {
      const userId = req.query.userId as string | undefined;

      if (!userId) {
        return res
          .status(400)
          .json({ error: "Paramètre 'userId' obligatoire." });
      }

      const rows = await db
        .select()
        .from(stockPositions)
        .where(eq(stockPositions.userId, userId));

      return res.status(200).json({ data: rows });
    }

    // ============================================================
    // POST — création d’une position d’action
    // ============================================================
    if (req.method === "POST") {
      const body =
        typeof req.body === "string"
          ? JSON.parse(req.body)
          : req.body || {};

      const {
        userId,
        symbol,
        name,
        exchange,
        logoUrl,
        quantity,
        buyPrice,
        buyCurrency,
        buyDate,
        notes,
      } = body;

      if (!userId || !symbol || !quantity || !buyPrice || !buyCurrency || !buyDate) {
        return res.status(400).json({
          error:
            "Champs obligatoires manquants : userId, symbol, quantity, buyPrice, buyCurrency, buyDate.",
        });
      }

      const buyTotal = Number(quantity) * Number(buyPrice);

      const [inserted] = await db
        .insert(stockPositions)
        .values({
          userId,
          symbol,
          name: name || null,
          exchange: exchange || null,
          logoUrl: logoUrl || null,
          quantity: quantity.toString(),
          buyPrice: buyPrice.toString(),
          buyTotal: buyTotal.toString(),
          buyCurrency,
          buyDate,
          notes: notes || null,
        })
        .returning();

      return res.status(200).json({ success: true, row: inserted });
    }

    // ============================================================
    // DELETE — supprimer une position
    // ============================================================
    if (req.method === "DELETE") {
      const body =
        typeof req.body === "string"
          ? JSON.parse(req.body)
          : req.body || {};

      const { id, userId } = body;

      if (!id || !userId) {
        return res
          .status(400)
          .json({ error: "Champs 'id' et 'userId' obligatoires pour DELETE." });
      }

      await db
        .delete(stockPositions)
        .where(eq(stockPositions.id, id));

      return res.status(200).json({ success: true });
    }

    // Méthode non autorisée
    return res.status(405).json({
      error: `Method ${req.method} not allowed. Use GET / POST / DELETE.`,
    });
  } catch (err: any) {
    console.error("Error in /api/stocks:", err);
    return res
      .status(500)
      .json({ error: err?.message || "Erreur serveur /api/stocks" });
  }
}
